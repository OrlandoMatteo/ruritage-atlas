...
with open('dbCredentials.json') as f:
	...
	replicatorArea=client.ruritage.replicatorArea
        rmPath=client.ruritage.rmPath
	rmBuildings=client.ruritage.rmBuildings
        rmHistBuildings=client.ruritage.rmHistBuildings
	rBuildings=client.ruritage.replicatorBuildings
        rHistBuildings=client.ruritage.replicatorHistBuildings
        rmTowns=client.ruritage.rmTowns
	photos=client.ruritage.images
	sites=client.ruritage.sites
	rmSites=client.ruritage.rmSites
	rSites=client.ruritage.replicatorSites
...
hist_bui_types=[]
for x in rmHistBuildings.find({}):
	if 'BUI_TYP' in x['properties'].keys():
		hist_bui_types.append(x['properties']['BUI_TYP'])
hist_bui_types=list( dict.fromkeys(hist_bui_types) )		
...
@app.route('/test')
def test():
	return render_template('test.html', buiTypes=bui_types, histBuiTypes=hist_bui_types)
...
@app.route('/RAreas',methods=['GET','POST'])
def RAreas():
	output={}
	output['data']=[]
	invalid={'_id'}
	for pol in replicatorArea.find({},no_cursor_timeout=True):
		area={x: pol[x] for x in pol if x not in invalid}	
		output['data'].append(area)	
	#output={}
	#output['data']=[]
...
def buildings():
	...
	for point in rBuildings.find({'coordinates':{ '$geoWithin': { '$geometry': rectangle } }, 'properties.BUI_TYP':{'$in':types} },no_cursor_timeout=True):
		building={x: point[x] for x in point if x not in invalid}
		output['features'].append(building)
...
@app.route('/histbuildings',methods=['GET','POST'])
def histbuildings():
	bounds=ast.literal_eval(request.args.get('bounds').encode('UTF8'))
	types=ast.literal_eval(request.args.get('bui_types').encode('UTF8'))
	rectangle={
	...
	output={ "type": "FeatureCollection", "features": []}
        print types
	for point in rmHistBuildings.find({'coordinates':{ '$geoWithin': { '$geometry': rectangle } }, 'properties.BUI_TYP':{'$in':types} },no_cursor_timeout=True):
		building={x: point[x] for x in point if x not in invalid}
		output['features'].append(building)
        
	for point in rHistBuildings.find({'coordinates':{ '$geoWithin': { '$geometry': rectangle } }, 'properties.BUI_TYP':{'$in':types} },no_cursor_timeout=True):
		building={x: point[x] for x in point if x not in invalid}
		output['features'].append(building)
...
def Sites():
	...
	for point in rSites.find({},no_cursor_timeout=True):
		site={x: point[x] for x in point if x not in invalid}
		output['features'].append(site)
        
	for point in rmSites.find({},no_cursor_timeout=True):
		site={x: point[x] for x in point if x not in invalid}
		output['features'].append(site)
